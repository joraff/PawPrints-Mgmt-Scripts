...
require 'c:\PCOUNTER\DATA\Scripts\dbconnect.rb'
require 'file/tail'
require 'digest/md5'
...
File::Tail::Logfile.tail(logfile, :backward => limit, :interval => 1, :max_interval => 3) do |line|
  if counter == 1 
    counter += 1
    next
  end
  ...
  if counter <= limit
    ...
    puts "inserting new job from #{job['username']}"
    History.create(job)
  end
end

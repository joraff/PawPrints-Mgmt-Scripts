...
$input_mode=0;
$config="PCounter.cfg";
$path_to_pcounter=q{C:\Program Files (x86)\Pcounter for NT\NT\ACCOUNT.EXE};
...
#  print "DEBUG: $_";
  ...
  if (defined ( my $id_row_ref = $budb_statement_handle->fetchrow_arrayref() )) {
	...
	do { &LBDie(2, $law_statement_handle->err.
                 ": LAW STATUS lookup failed for $bid $@\n"); }
	if ($@ || $law_statement_handle->err);
	
	if (defined ( my $id_row_ref = $law_statement_handle->fetchrow_arrayref() )) {
		($stat_ind) = @{ $id_row_ref };
		
		# we only want to bill students ... not any staff, faculty, law, or others
		...
		# check for and exclude law students
		if( ($stat_ind =~ m/L/i) ) {
			print LBLOG "Not billed (law student): $bid,$pcount,S=$stu_ind,Fac=$fac_ind,Stf=$stf_ind,Aux=$aux_ind,Spc=$spc_ind,Law=$stat_ind\n";
			next PCOUNT_REC;
		}
